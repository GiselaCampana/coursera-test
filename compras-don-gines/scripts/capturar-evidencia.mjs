    ...
    for (const { zona, region } of ZONAS) {
      for (const { variante, receta } of VARIANTES) {
        ...
        process.stderr.write(
          `${comprobante.nombre} ${id}: ${fragmentos.length} fragmentos acumulados (${ms} ms)\n`,
        );
      }
    }

    const evidencia = { anchoPx, altoPx, pasadas, fragmentos };
    const destino = path.join(SALIDA, `${comprobante.nombre}.json`);
    writeFileSync(destino, JSON.stringify(evidencia));
    process.stderr.write(
      `→ ${destino}: ${fragmentos.length} fragmentos, ${pasadas.length} pasadas, ` +
        `${Math.round((Date.now() - comienzoFactura) / 1000)} s\n`,
    );
  }
