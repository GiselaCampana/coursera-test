...
export async function priceRowsToXlsx(rows: PriceExportRow[]): Promise<Buffer> {
  ...
  const data: Array<Array<string | number | null>> = [
    ...
    ...rows.map((r) => [
      ...
      Number(r.alCorteHormaDigitalMarginPct),
      Number(r.alCorteHormaCashMarginPct),
      Number(r.alCorteCajaCashMarginPct),
      Number(r.feteado100gMarginPct),
      Number(r.feteadoQuarterMarginPct),
      Number(r.feteadoPieceDigitalMarginPct),
      Number(r.feteadoPieceCashMarginPct),
      Number(r.wholeUnitMarginPct),
      n(r.salePricePerKg),
