/**
 * Segunda salida cruda de Tesseract sobre Safari, en un iPhone de verdad.
 *
 * Misma foto que `errecalde-safari.ts`, misma versión del lector, **otro
 * resultado**. No es un capricho: el recorte de la tabla no cae dos veces en el
 * mismo lugar, el análisis de disposición parte las filas en otros puntos y los
 * separadores se pierden en renglones distintos. Por eso hacen falta los dos
 * fixtures y la aceptación tiene que pasar con los dos.
 *
 * Lo que esta corrida trae de nuevo, y que la anterior no tenía:
 *
 *  - **El recorte de la tabla se corta antes de terminar**: la última fila que
 *    trae es SARDO DON ALFONSO. TOMATE EN BOTELLA no aparece por ningún lado
 *    con su código ni su descripción.
 *  - **De TOMATE sólo queda un jirón en la página completa**, justo antes del
 *    pie: `2             0% — 21% $3268324`. Tiene forma de renglón —una
 *    cantidad, el 0 %, el 21 % y un subtotal— pero no tiene con qué
 *    identificarse. Su precio, $1.021,35, **no está en ningún lado de este
 *    texto**: de acá no se lo puede reconstruir, sólo detectar que falta.
 *  - **SARDO BLOQUE viene una sola vez y degradado**: `L 3 A75kg  $13.29525 0%
 *    21%  $6315243`. En la corrida anterior había además una lectura limpia con
 *    la que compararlo; acá no hay con qué, y hay que resolver la escala contra
 *    el neto impreso.
 *
 * Los dos son el modo de fallar peligroso del OCR: perder una fila entera y que
 * nadie la extrañe, y leer un renglón de $6,3 M en una factura de $3,83 M.
 *
 * Está copiado tal cual, sin corregir una letra.
 */

export const SAFARI2_ARTICULOS = `copIGO   DESCRIPCION                               UNID. CANTIDAD
...
export const SAFARI2_COMPLETO = `ERRECALDE
...
export const SAFARI2_TEXTOS = {
  completo: SAFARI2_COMPLETO,
  articulos: SAFARI2_ARTICULOS,
};

/** Lo que dice el papel. Es lo mismo que esperan los otros dos fixtures. */
export const SAFARI2_ESPERADO = {
  renglones: 23,
  netTotal: '3830467.37',
  ivaTotal: '804398.16',
